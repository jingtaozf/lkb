...
;;;
;;; disable active parsing mode by default until more testing has been done.
;;;                                                        (20-jun-99  -  oe)
;;;
...
(defun initialize-arules ()
  ...
  (loop
      ...
      do
        (push arule *arules*)))
        
(defparameter *key-daughter-path* '(key-arg))
            ...
            (let* ((tdfs (yadu rtdfs (create-temp-parsing-tdfs ptdfs path))))
              (when tdfs
                (if open
                  ...
                  (restrict-and-copy-tdfs (tdfs-at-end-of mother tdfs)))))))
         (nedge (and result
                     (make-edge :id (next-edge)
