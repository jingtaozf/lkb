...
\usepackage{latexsym}
%\usepackage{times}
% should use times but too many missing fonts
...
\item Integration with the TSDB test suite machinery.
...
The full LKB system is available in four configurations:
\begin{enumerate}
\item Solaris image
\item Mac image
...
We would suggest 32 Meg of RAM as a minimum on MCL. To run larger grammars
you will need larger amount of memory.  The size of the source
files is about (FIX whatever ...)
The Solaris image will run on (FIX what?)
...
We have tested the system on Allegro Common Lisp 4.3 and 5.0 on Sun Solaris 
(FIX), 
and on 
Macintosh Common Lisp 2.0.1, 3.9 and 4.0, under MacOS (what? FIX).
...
your own local copy of the LKB system source. 
...
The image files are available from the
\href{http://www-csli.stanford.edu/~aac/lkb.html}{LKB Web Site}.
Download the file, making sure you are using binary mode.
Then:
\begin{verbatim}
gunzip newlkb.gz
\end{verbatim}
this should give you an image file called {\tt newlkb}.  

Now follow the instructions in \S\ref{clim-image} or 
\S\ref{mcl-image} to start the LKB.
...
\footnote{Note that, for simplicity, we describe the procedure for
...
\item At this point, you have a choice:
if you want to make a Lisp image, to simplify later loading,
execute \verb+(dump-lkb)+.
You can start the LKB system from the image as described in 
\S\ref{emacs-image}.
% FIX ??
Otherwise, once you have exited Lisp,
...
if you want to make an image, to simplify later loading,
execute (dump-lkb).  This will end your Lisp session.
You can restart the LKB from the image as described in \S\ref{mcl-image}.
% may not on later versions of MCL?
% FIX ??
Otherwise, once you have exited Lisp 
you can restart the system from the compiled files
as described in 
...
files in ACL/CLIM, you must use the emacs text editor. 
...
file
\begin{verbatim}
FIX - don't know whether we can give instructions
to do something adequate outside CSLI?
...
\item Start ACL/CLIM from within emacs.
At CSLI, the command is \verb+fi:common-lisp+ which takes a number
of arguments. The
default values should be accepted with the exception
of the image name, as follows:
\begin{quote}
...
\end{verbatim}
\item Start emacs
\item Start the LKB image by (whatever FIX)
\end{enumerate}
You
should now see the LKB interaction window, as shown in 
Figure~\ref{lkbtop-unix}.
...
simply start the saved image by running {\tt newlkb} (or whatever
...
In case of installation or other problems, please look at the instructions
on the 
\href{http://www-csli.stanford.edu/~aac/lkb.html}{LKB Web Site}.
We will make fixes for known problems available there.
The Web site also contains details of how to report bugs that you find:
note that it's important that you give us enough information,
so the Web site describes what information we need.
...
Select {\bf Load grammar} from the LKB {\bf Load}
...
With the toy grammar, there should be no errors.  If
you do get error messages when trying to load the {\tt toy} script, it is
...
\begin{figure}
\epsfxsize=1in
\epsfbox[0 0 200 200]{figs/grule.ps}
...
The view commands let you get an entities such as lexical entries which cannot
...
ACL/CLIM only.
This closes the LKB top window.  It may be reopened by
...
more functionality is available from the TSDB machinery which
...
and relies on the use of an MRS style semantics.  At the moment,
...
the current system, finishing the Lisp session.  
The session can then be restarted by double-clicking on the new image file.
The dumped system relies on the cached lexicon, so if the templex
or templex-index files have been deleted or modified, the saved image will
not work correctly.
...
use of test suites (see e.g., Flickinger and Oepen (19??)).
%FIX
The LKB system is compatible with the TSDB system.
...
(setf *psorts-temp-file* (temporary-file "biglex")) 
(setf *psorts-temp-index-file* (temporary-file "biglex-index"))
        ;; so doesn't conflict with another lexicon
...
\item The
specification of a name for the
temporary lexicon file and its associated index file
\verb+*psorts-temp-file*+ and \verb+*psorts-temp-index-file*+: 
ensuring that the lexicon file associated with a grammar has a 
unique name allows
multiple grammars to be used without overriding another lexicon
(see \S\ref{glob} and \S\ref{cache}).
...
but it could have been stored in the temporary directory.
...
\lispcommand{temporary-directory}

Returns the user's
temporary directory, as set up in {\tt general/loadup-library}.
By default, this is {\tt $\sim$/tmp}.  (Note that it is important that each
user have a distinct temporary directory.)

\lispcommand{temporary-file {\it name}}

Specifies a file {\it name} in the temporary directory.
...
rather than in memory.  Lexical entries
...
much more sophisticated test-suite support is available
by using the TSDB system.  See FIX ref whatever
...
\end{verbatim}
where the value of \verb+mk::tmp-dir+ is set in the loadup-library.

\item[*psorts-temp-index-file*, {\tt $\sim$/tmp/templex-index}]
...
\item[make-orth-fs] A function used by the parser
...
%MRS semantics (Copestake et al, 1997)
