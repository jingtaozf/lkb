...
system will need a license so that they can work with the source files.
...
\end{verbatim}
this should give you a file called \filename{lkbsrc5-0.tar}.  
...
\end{verbatim}
this should give you a file called \filename{climimage5-0.tar}.
...
at CSLI, the command to start ACL with CLIM is \filename{/usr/acl/bin/acl}.
...
and you want to use the folder name \filename{lkb5.0}
...
and stored it in the directory \filename{lkb}, accept all the default
...
{\bf Image arguments:}  \verb+<RET>+
\end{quote}
(if you are working with an image you have saved via {\lkbmenucommand Dump lkb}, 
...
You should be able to fix this by adding \filename{/usr/dt/lib} to your {\tt
...
Select {\lkbmenucommand View} from the LKB top menu and then select {\lkbmenucommand Word entries}.
...
To parse a sentence, click on {\lkbmenucommand Parse} / {\lkbmenucommand Parse input}.  Unless you have been parsing other things already,
...
{\lkbmenucommand Feature structure --- Edge 3}.  You will
...
Now try {\lkbmenucommand Parse} / {\lkbmenucommand Batch Parse}.  You will be prompted for the
...
the editor, it should show the number of parses (if
any) and the number of edges (phrases
constructed while attempting to parse) for each sentence
and a time for the total processing.
...
and {\type swimmer}.  Now {\type vertebrate} and {\type swimmer} 
have multiple mutual descendants which don't stand 
in an inheritance relationship: {\type fish} and {\type whale}.
...
{\type whale} is
not a descendant of {\type fish}.  In fact the number of
...
are reentrant in F are also reentrant in G.
\end{description}
...
that one if the structures specifies that a node at the end of 
some path P
...
to descriptions (e.g., of lexical entries etc) won't
...
We can achieve this by given a definition of the
...
There are a series of conditions on type constraints
...
Although the constraints on the grammars we've been
...
types are given names such as {\type glbtype1} etc.  They never have constraints.
...
have two buttons: {\lkbmenucommand Close} and {\lkbmenucommand Close all}.
...
\lkbmenucommand{Generate} from the appropriate window.
...
for a list of features (see \S\ref{FIX} for a discussion of maximal types).
...
In MCL, choosing {\lkbmenucommand Dump system} will prompt for a file name for the saved
...
grammar.  
In fact, the parse is blocked by the initial structure check
...
if the parameter \lkbparam{*dont-show-morphology*} is nil, but not shown otherwise.
Similarly the parameter \lkbparam{*dont-show-lex-rules*} controls whether or
...
interface to a lexicon constructed automatically from COMLEX 
(see \ref{comlex}). 
...
%FIX - file names set by function
Whenever a lexicon is read into the LKB, it is stored in an external file
(specified by the parameter \lkbparam{*psorts-temp-file*}, see \S\ref{sysglob})
...
(specified by \lkbparam{*psorts-temp-index-file*}, see \S\ref{sysglob}) so that
they do not need to be recreated when the grammar is read in again if the
lexicon has not changed.  The script command
...
can be also be run with the {\lkbmenucommand Index} command
...
click {\lkbmenucommand Generate \ldots} in the {\lkbmenucommand Generate} menu and give this
...
This class of parameters can be set via the {\lkbmenucommand Options}
...
{\lkbmenucommand Options} menu are all ones that are specified as being
...
\begin{description}
\item[*psorts-temp-file*, \filename{$\sim$/tmp/templex}]
