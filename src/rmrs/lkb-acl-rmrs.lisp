...
(defparameter *qa-question-nos* '(3 4 5 6 7 20 21 22 23 24))

(defparameter *qa-questions*
'(    
("What does the Peugeot company manufacture ?" . 3)
("How much did Mercury spend on advertising in 1993 ?" . 11)
("What is the name of the managing director of Apricot Computer ?" . 1)
("Why did David Koresh ask the FBI for a word processor ?" . 1)
("What debts did Qintex group leave ?" . 8)
("Where is Inoco based ?" . 1)
("Who was the first American in space ?" . 1)
("When did the Jurassic Period end ?" . 3)
("When did Spain and Korea start ambassadorial relations ?" . 1)
("When did Nixon visit China ?" . 1)
))


#+:lkb
(defun qa-qs nil
  (let ((qa-question-nos *qa-question-nos*))
  (dolist (q *qa-questions*)
    (let ((sentence (car q))
          (parsenum (cdr q))
          (filename (format nil "q~A.rmrs" (car qa-question-nos))))
      (with-open-file (ostream filename :direction :output
                       :if-exists :supersede)
        (output-header-blah ostream)
        (format ostream "~%<S id='~A'>" (car qa-question-nos))
        (setf qa-question-nos (cdr qa-question-nos))
        (if sentence
            (format ostream
                    "~%<string>~%~S~%</string>" sentence)
          (format ostream
