...
;;; 26-MAY-93 iln   Added new functionality: Two new arguments for
;;;                 operate-on-system (and load- and compile-system resp.) that
;;;                 control the execution of initially-do and finally-do forms:
;;;                 :execute(initially|finally)-do. The default for these is
;;;                 stored in the global variables *execute-initally-do* and
;;;                 *execute-finally-do*.
;;;                 The second change affects the definition of defsystem:
;;;                 A new slot :operation-form was added to make it possible to
;;;                 override the "default" action associated with an operation.
;;;                 This is useful in cases were for example a
;;;                 compiler-compiler is used to compile a grammar file and
;;;                 generates a plain lisp file out of it.
;;;  5-Aug-93 bk    Fixed Problem with Allegro 4.2 that doesn't accept
;;;                 (pathname-device NIL) 
...
#+(and :excl :allegro-version>= :cltl2)
...
(defvar *dont-redefine-require* T  ;; we don't want it *** bk 28.06.94
...
;;; ILN 93-05-24
(defvar *execute-initially-do* T
  "Switches :initialliy-do on or off.")
(defvar *execute-finally-do* T
  "Switches :finally-do on or off.")
...
;;; ILN
  operation-form                        ; assoc list for redefining operations
  ...
  (when component
	...
	(setf pathname (lp:translate-logical-pathname pathname)))
      ...
      (make-pathname :name (pathname-name pathname)
		     :type (unless
			       (and 
				(stringp (component-extension component type))
				(zerop (length
					(component-extension component type))))
			     (component-extension component type)) 
...
;;; ILN                                              
                               (execute-initially-do *execute-initially-do*)
	...
	(progn
	  ...
	  (when (eq type :defsystem)	; maybe :system too?
	    (operate-on-system-dependencies component operation force))
          ...
          ;; Do any initial actions (if not inhibited)
	  (when (and *execute-initially-do*
		     (component-initially-do component))
...
;;; ILN
	  ;; now save the current operation and change to a explicitely
	  ;; specified :operation-form (if there is any). Execute the correct
	  ;; operation and reset the old operation after recursion (by using an
	  ;; unwind-protect to prevent from destroying the "default"
	  ;; operations)
          (let ((current-op (component-operation operation))
                (new-defined-op 
                 (second
		  (assoc operation 
			 (component-operation-form component)))))
	    (unwind-protect
		(progn
		  (when new-defined-op
		    (component-operation
		     operation
		     (if (functionp new-defined-op)
			 new-defined-op
		       (eval `(function ,new-defined-op)))))
	      ...
	      (component-operation operation current-op)))

          ;; Do any final actions (if not inhibited)
	  (when (and *execute-finally-do*
		     (component-finally-do component))
