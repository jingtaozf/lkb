...
(defmacro DEFPACKAGE (name &rest options)
  ...
  (loop for (option) in options
	unless (member option '(:documentation :size :nicknames :shadow :shadowing-import-from :use :import-from :intern :export :export-from :add-use-defaults))
  ...
  (labels ((option-test (arg1 arg2) (when (consp arg2) (equal (car arg2) arg1)))
    ...
    (let ((nicknames (mapcar #'string (option-values ':nicknames options)))
	 ...
	 (let ((*package* (find-package ,name)))
	   ...
	   (USE-PACKAGE ',(append (mapcar #'string (option-values ':USE options)) '("COMMON-LISP" #+:lucid "LUCID-COMMON-LISP")))
