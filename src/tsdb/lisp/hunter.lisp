...
(defun sign-test-on-sequences (sequence1 sequence2 &key (exact? nil) (tails :both))
  ...
  (let* ((differences (map 'list #'- sequence1 sequence2))
         (plus-count (count-if #'plusp differences))
         (minus-count (count-if #'minusp differences)))
    ;;; buf-fix: changed count() to count-if() in the let bindings above,
    ;;; erikve 
...
(defun wilcoxon-signed-rank-test-on-sequences (sequence1 sequence2
                                               &key (tails :both))
  ;; changed &optional to &key in order to make the argument list 
  ;; conform to that of all the other *-on-sequences test functions
  ;; erikve 1/6-2007
