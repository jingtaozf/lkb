...
(defvar *tsdb-exhaustive-p* t)

(defun parse-item (string &key (exhaustive *tsdb-exhaustive-p*)
                   (tasks 0) trace derivations semantix-hook trees-hook)
  (declare (ignore exhaustive tasks trace derivations semantix-hook trees-hook))
  (let* ((*package* (find-package "COMMON-LISP-USER"))
         (sent
            ...
            (multiple-value-prog1
               ...
               (let ((rawgc (- #+mcl (ccl:gctime) tgc)))
                  (setq symbols 0 conses 0
                        ...
                        tcpu
                        (round (* (- (get-internal-run-time) tcpu #+mcl rawgc) 100)
         ...
         (let ((n 0)
            ...
            (multiple-value-bind (l-s-tasks redges words)
               ...
               `((:TIMEUP) (:COPIES . -1) (:UNIFICATIONS . -1)
                 ...
                 (:TREAL . ,treal) (:TCPU . ,(+ tcpu tgc)) (:TGC . ,tgc)
                 (:REDGES . ,redges) (:PEDGES . ,*edge-id*) (:AEDGES . -1)
                 (:P-STASKS . ,s-tasks) (:P-ETASKS . ,e-tasks) (:P-FTASKS . ,f-tasks)
                 (:P-CTASKS . ,c-tasks) (:L-STASKS . ,l-s-tasks) (:WORDS . ,words)
                 (:TOTAL . ,tcpu) (:FIRST . ,tcpu) (:READINGS . ,(length *parse-record*))
                 (:ERROR .
                   ,(substitute #\; #\newline
                      (remove #\@
                    ...
                    ,(mapcar
                        #'(lambda (parse)
                            (prog1
                               `((:MRS . "") (:TREE . "")
                                 (:DERIVATION .
                                     ,(prin1-to-string (parse-tree-structure parse)))
                                 (:R-REDGES . ,(length (parse-tsdb-distinct-edges parse nil)))
                                 (:SIZE . ,(parse-tsdb-count-nodes parse))
                                 (:R-STASKS . -1) (:R-ETASKS . -1) (:R-FTASKS . -1)
                                 (:R-CTASKS . -1) (:TIME . ,tcpu) (:RESULT-ID . ,n))
                               (setq tcpu 0 n (1+ n))))
                        *parse-record*))))))))
